...
router.get("/", async(req, res) => {
    ...
    try {
        ...
        const allBooks = await pool.query("SELECT * FROM pagto");
...
router.get("/:id", async(req, res) => {
    try {
        const { id } = req.params;
        const getBook = await pool.query("SELECT * FROM pagto WHERE cpf = $1",[ id ]);
...
router.put("/:id", async(req, res) => {
    try {
        ...
        const updBook = await pool.query(
            "UPDATE pagto SET title = $1, primary_author = $2 WHERE cpf = $3 RETURNING *",
            [ title, author, id ]
        );
